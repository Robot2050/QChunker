...
parser.add_argument('--out_path', type=str, default='huagong/lumberchunker.json', help='Output directory path')
...
with open('tmp_cleandata/huagong.json', 'r', encoding='utf-8') as file:  
    qa_data = json.load(file)
for zdata in qa_data:
    segments = split_text_by_punctuation(zdata,'zh')       
...
while chunk_number < len(id_chunks)-5:
    ...
    while word_count < 350 and i+chunk_number<len(id_chunks)-1:
        ...
        word_count = count_words(final_document,'zh')   
...
with open(out_path, 'w',encoding='utf-8') as sfile:
...
# CUDA_VISIBLE_DEVICES=1 nohup python chunk_lumberchunker.py >> huagong/lumberchunker.log 2>&1 &
